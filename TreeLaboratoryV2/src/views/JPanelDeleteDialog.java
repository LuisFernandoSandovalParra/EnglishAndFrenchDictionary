...
public class JPanelDeleteDialog extends JPanel implements ChangeListener {
	...
	public JPanelDeleteDialog(ActionListener actionListener) {
	...
	private void initComponents(ActionListener actionListener) {
		createTitleDeleteWord();
		createFormularyDeleteWord();
		createButtons(actionListener);
	}

	private void createTitleDeleteWord() {
		JLabel titleDialog = new JLabel(ConstantsGUI.TEXT_DELETE_WORD_TITLE, JLabel.CENTER);
	...
	private void createFormularyDeleteWord() {
		JPanel formPanel = new JPanel();
		formPanel.setBackground(Color.WHITE);
		formPanel.setBorder(ConstantsGUI.BORDER_ADD_WORD_FORM);
		formPanel.setLayout(new GridLayout(2, 2, 0, 20));
	...
	public void createButtons(ActionListener actionListener) {
		...
		acceptBtn.setActionCommand(Event.DELETE_WORD.name());
